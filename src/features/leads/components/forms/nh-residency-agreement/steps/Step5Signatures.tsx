...
function useSignature(
  ...
  // Detect pre-existing signature from API using the React
  // "adjusting state during render" pattern (avoids setState-in-effect).
  // Only triggers once, before the user has interacted with the controls.
  const isApiValue =
    watchValue &&
  ...
  if (!hadExistingFromApi && !userInteracted && isApiValue) {
    ...
    // Restore the correct tab based on saved method
  ...
  if (lockedMessage) {
    return (
      <Grid size={{ xs: 12 }} sx={{ mb: 4 }}>
        ...
        {/* Show existing signature from API if available */}
          ...
          <Box sx={sigContainerSx}>
            <Box sx={headerRowSx}>
              ...
              <CustomButton
                ...
                onClick={handleClear}
                disabled={
                  isViewMode || (!hasDrawn && !hadExistingFromApi)
                }
              >
                Clear
              </CustomButton>
...
/* ── Main component ── */
const Step5Signatures: React.FC<Step5Props> = ({
  control,
  setValue,
  watch,
  mode,
  ...
  const residentValue = watch("residentSignature");
  const guardianValue = watch("legalGuardianSignature");
  const providerValue = watch("providerSignature");
  const residentMethodValue = watch("residentSignatureMethod");
  const guardianMethodValue = watch("legalGuardianSignatureMethod");
  const providerMethodValue = watch("providerSignatureMethod");
  ...
  return (
    <Grid container spacing={2}>
      ...
      {/* Individual/Resident - Separate Card */}
      <Grid size={{ xs: 12 }} sx={{ mb: 3 }}>
        ...
        >
          <SignatureSection
            ...
            signatureFieldName="residentSignature"
            dateFieldName="residentSignatureDate"
            control={control}
            setValue={setValue}
            watchValue={residentValue}
            watchMethodValue={residentMethodValue}
      ...
      <Grid size={{ xs: 12 }} sx={{ mb: 3 }}>
        ...
        >
          <SignatureSection
            ...
            signatureFieldName="legalGuardianSignature"
            dateFieldName="legalGuardianSignatureDate"
            control={control}
            setValue={setValue}
      ...
      <Grid size={{ xs: 12 }}>
        ...
        >
          <SignatureSection
            title="Provider"
            signatureFieldName="providerSignature"
            dateFieldName="providerSignatureDate"
            control={control}
            setValue={setValue}
            watchValue={providerValue}
            watchMethodValue={providerMethodValue}
            isViewMode={isViewMode}
          />
        </Grid>
      </Grid>
...
export default Step5Signatures;